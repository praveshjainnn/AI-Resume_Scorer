...
async def analyze_resume(
    ...
    try:
        logger.info("[PIPELINE] Starting file read...")
        file_bytes = await resume.read()
        filename   = resume.filename or 'resume'
        logger.info(f"[PIPELINE] File read complete. Size: {len(file_bytes)} bytes. Starting parse...")
        ...
        logger.info(f"[PIPELINE] Parse complete. Text length: {len(resume_text)} characters. Starting analyze_full_resume...")

    except Exception as exc:
        logger.error(f'[PIPELINE] File parsing failed: {exc}')
        raise HTTPException(
            status_code=422,
    ...
    try:
        ...
        logger.info("[PIPELINE] Calling analyze_full_resume...")
        ...
        logger.info("[PIPELINE] analyze_full_resume complete!")
    except Exception as exc:
        logger.error(f'[PIPELINE] Full analysis pipeline failed: {exc}')
...
async def generate_pdf(
    ...
    from backend.services.pdf_export import generate_combined_pdf, WEASYPRINT_INSTALLED
    from fastapi.responses import Response

    if not WEASYPRINT_INSTALLED:
        from backend.services.pdf_export import generate_fpdf_report
        try:
            pdf_bytes = generate_fpdf_report(data.model_dump())
            return Response(
                content=pdf_bytes,
...
async def generate_history_pdf(
    ...
    if not WEASYPRINT_INSTALLED:
        from backend.services.pdf_export import generate_fpdf_report
        try:
            pdf_bytes = generate_fpdf_report(analysis_data)
            return Response(
                content=pdf_bytes,
                media_type="application/pdf",
                headers={
