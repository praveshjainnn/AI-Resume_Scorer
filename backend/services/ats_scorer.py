...
def validate_skills_with_projects(
    ...
    # Pre-encode all skills in a single batch
    skill_embeddings = {}
    skills_clean = [s for s in skills if s]
    if skills_clean:
        try:
            vectors = embedder.encode(skills_clean, convert_to_tensor=False)
            for s, vec in zip(skills_clean, vectors):
                skill_embeddings[s] = vec
        except Exception as e:
            log_warning(f"Failed to batch-encode skills: {e}", context='ats_scorer')

    # Pre-encode all project texts
    project_embeddings = []
    for project in projects:
        ...
        if project_text.strip():
            try:
                vec = embedder.encode(project_text, convert_to_tensor=False)
            except Exception as e:
                log_warning(f"Failed to encode project text: {e}", context='ats_scorer')
        project_embeddings.append((project, project_text, vec))

    # Pre-encode experience text
    exp_vec = None
    if experience_text:
        try:
            exp_vec = embedder.encode(experience_text, convert_to_tensor=False)
        except Exception as e:
            log_warning(f"Failed to encode experience text: {e}", context='ats_scorer')

    validated_skills      = []
    unvalidated_skills    = []
    skill_project_mapping = {}

    def _get_similarity(s_vec, t_vec) -> float:
        if s_vec is None or t_vec is None:
            return 0.0
        try:
            similarity = np.dot(s_vec, t_vec) / (
    ...
    for skill in skills:
        matching_projects = []
        max_similarity    = 0.0
        s_vec = skill_embeddings.get(skill)

        # Check project matches
        for project, project_text, p_vec in project_embeddings:
            # Fast case-insensitive substring check first
            if skill.lower() in project_text.lower():
                matched, sim = True, 1.0
            else:
                sim = _get_similarity(s_vec, p_vec)
        ...
        if experience_text:
            if skill.lower() in experience_text.lower():
                ...
                sim = _get_similarity(s_vec, exp_vec)
                matched = sim >= threshold
